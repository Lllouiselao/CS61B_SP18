...
public class    DebugExercise2 {
    ...
    public static int max(int a, int b) {
        /** this original function didn't return the bigger one
         *  should change it or it will return the small one*/
        if ( a > b){
            return  a;
        }
        else{
    ...
    public static int arraySum(int[] x) {
        ...
        int sum = 0;
        while (i < x.length) {
            /** should just use add function or it will add the x[i]*/
            sum = add(sum, x[i]);
            i = i + 1;
        }
        return sum;
